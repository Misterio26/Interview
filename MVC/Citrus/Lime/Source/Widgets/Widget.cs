...
#region Misc properties
		...
		private Widget topmostParent;
		public Widget TopmostParent
		{
			get => topmostParent;
			set {
				if (topmostParent == value) {
					return;
				}
				var oldParent = topmostParent;
				topmostParent = value;

				foreach (var node in Nodes) {
					if (node is Widget widget) {
						widget.TopmostParent = topmostParent;
					}
				}

				OnTopmostParentChanged(oldParent);
				foreach (var component in Components) {
					component.OnTopmostOwnerChanged(oldParent);
				}
			}
		}

		protected virtual void OnTopmostParentChanged(Widget oldParent) { }
		...
		protected override void OnParentChanged(Node oldParent)
		{
			...
			TopmostParent = !(Parent is Widget) ? this : ((Widget)Parent).TopmostParent;
