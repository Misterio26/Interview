...
namespace Lime
{
	...
	public abstract class CustomWidgetPresenter : IPresenter
	{
		private class RenderObject : WidgetRenderObject
		{
			private RenderObjectList renderObjectList;

			public void Init(RenderObjectList renderObjectList)
			{
		...
		public Lime.RenderObject GetRenderObject(Node node)
		{
			var nodeAsWidget = node.AsWidget;
			var toPresentWidget = GetCustomToPresentWidget(node);

			if (toPresentWidget == null || nodeAsWidget == null) {
				return null;
			}

			var renderObjectList = new RenderObjectList();

			var savedPosition = toPresentWidget.Position;
			float savedRotation = toPresentWidget.Rotation;
			var savedScale = toPresentWidget.Scale;

			var widgetToWorld = Matrix32.Transformation(
				Vector2.Zero,
				savedScale,
				savedRotation * Mathf.DegToRad,
				savedPosition
			) * nodeAsWidget.LocalToWorldTransform;

			var transform2 = widgetToWorld.ToTransform2();

			toPresentWidget.Position = transform2.Translation;
			toPresentWidget.Rotation = transform2.Rotation;
			toPresentWidget.Scale = transform2.Scale;

			toPresentWidget.RenderChainBuilder.AddToRenderChain(renderChain);
			renderChain.GetRenderObjects(renderObjectList);
			renderChain.Clear();

			var renderObject = RenderObjectPool<RenderObject>.Acquire();
			renderObject.CaptureRenderState(toPresentWidget);
			renderObject.Init(renderObjectList);

			toPresentWidget.Position = savedPosition;
			toPresentWidget.Rotation = savedRotation;
			toPresentWidget.Scale = savedScale;

			return renderObject;
		}

		protected abstract Widget GetCustomToPresentWidget(Node node);
		public abstract bool PartialHitTest(Node node, ref HitTestArgs args);
		public abstract IPresenter Clone();
	}
}
