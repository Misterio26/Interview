	...
	class TextRenderer
	{
		...
		float CalcWordWidth(Word word, bool skipFirstLetter = false)
		{
			...
			Vector2 size = style.Font.MeasureTextLine(
				texts[word.TextIndex], ScaleSize(style.Size), word.Start, word.Length, style.LetterSpacing + style.Font.Spacing, skipFirstLetter);
				...
				for (int j = 0; j < count; j++) {
					...
					if (style.CastShadow) {
						for (int k = 0; k < (style.Bold ? 2 : 1); k++) {
							Renderer.DrawTextLine(
								...
								word.Start, word.Length, font.Spacing + style.LetterSpacing,
								spriteLists[word.Style], tag: word.Style, skipFirstLetter: word.IsTagBegin || word.LineBreak);
						}
					}
					...
					for (int k = 0; k < (style.Bold ? 2 : 1); k++) {
						Renderer.DrawTextLine(
							...
							word.Start, wordLength, font.Spacing + style.LetterSpacing,
							spriteLists[word.Style], tag: word.Style, skipFirstLetter: word.X == 0 || (word.IsTagBegin && IsBullet(word)));
					}
					c += wordLength;
				}
				// Draw overlays
				...
				if (
					...
					&& (
						word.X + font.MeasureTextLine(
							t.Substring(word.Start, 1), ScaleSize(style.Size), style.LetterSpacing + font.Spacing).X + dotsWidth > maxWidth
		...
		private int CalcFittedCharactersCount(Word word, float maxWidth)
		{
			...
			do {
				mid = min + ((max - min) / 2);
				var w = font.MeasureTextLine(t, ScaleSize(style.Size), word.Start, mid, style.LetterSpacing + font.Spacing).X;
